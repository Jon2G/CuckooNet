...
{
    public class CreateFileRequest : SubimtRequest<FileInfo>
    {
        /// <summary>
        ...
        public CreateFileRequest(FileInfo file,
        ...
        : base(file, package, timeOut, priority, options, machine, platform, tags, custom, owner, clock, memory, unique, enforceTimeout)
        {
        ...
        public override MultipartFormDataContent GetContent()
        {
            if (!this.Sample.Exists)
                throw new FileNotFoundException(Sample.FullName);
            MultipartFormDataContent multi = new MultipartFormDataContent();
            Stream stream = System.IO.File.Open(Sample.FullName, FileMode.Open, FileAccess.Read, FileShare.None);
            multi.Add(CuckooWebClient.CreateFileContent(stream, Sample.Name)); //The big file upload API does not like it when multi-part uploads contain the size field
