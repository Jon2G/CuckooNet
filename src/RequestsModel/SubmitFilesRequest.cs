...
{
    public class SubmitFilesRequest : SubimtRequest<FileInfo[]>
    {

        /// <summary>
        /// Adds a file to the list of pending tasks. Returns the ID of the newly created task.
        /// </summary>
        /// <param name="files">Sample files (multipart encoded file content)</param>
        ...
        public SubmitFilesRequest(FileInfo[] files,
        ...
        : base(files, package, timeOut, priority, options, machine, platform, tags, custom, owner, clock, memory, unique, enforceTimeout)
        {

        }
        public override MultipartFormDataContent GetContent()
        {
            if (this.Sample.FirstOrDefault(x => !x.Exists) is { } notFoundInfo)
                throw new FileNotFoundException(notFoundInfo.FullName);
            MultipartFormDataContent multi = new MultipartFormDataContent();
            foreach (FileInfo file in Sample)
            {
                Stream stream = System.IO.File.Open(file.FullName, FileMode.Open, FileAccess.Read, FileShare.None);
                multi.Add(CuckooWebClient.CreateFileContent(stream, file.Name)); //The big file upload API does not like it when multi-part uploads contain the size field
            }
